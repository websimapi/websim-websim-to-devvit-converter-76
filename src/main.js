...
exportTarget.addEventListener('change', () => {
    ...
    if (target === 'devvit') {
        document.getElementById('devvit-options').classList.remove('hidden');
        document.getElementById('web-options').classList.add('hidden');
        btnDownload.textContent = 'Download Devvit Project (.zip)';
    } else {
        document.getElementById('devvit-options').classList.add('hidden');
        document.getElementById('web-options').classList.remove('hidden');
        btnDownload.textContent = 'Download Web Bundle (.zip)';
    }
});
...
<body>
    ...
    ${jsFile ? `<script type="module" src="./${jsFile}"></script>` : ''}
        ...
        const result = await generateDevvitZip(currentProjectMeta, finalAssets, includeReadme, launchMode, target);
